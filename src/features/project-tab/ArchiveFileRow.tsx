    ...
    return (<div data-ui="archive-file-row.1" key={`${kind}-${file.id}-${catId}`} draggable onDragStart={onDragStart} onDragEnd={onDragEnd} style={{
      ...
      <div data-ui="archive-file-row.3" style={{ minWidth: 0 }}>
        ...
        <div data-ui="archive-file-row.5" style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2, flexWrap: 'wrap' }}>
          ...
          {kind === 'site_photo' && validation && <span style={{ fontSize: 11, fontWeight: 900, color: validation.status === 'suitable' ? C.ok : C.danger, background: validation.status === 'suitable' ? '#F4FBF6' : C.dangerBg, border: `1px solid ${validation.status === 'suitable' ? '#BFE6C8' : '#FFCDD2'}`, borderRadius: 999, padding: '4px 8px', lineHeight: 1.2, whiteSpace: 'nowrap' }}>{validation.status === 'suitable' ? '적합' : '부적합'}</span>}
        </div>
      </div>
