    ...
    <div data-ui="features-project-tab-archive-hierarchy-view.layout" style={{ display: 'grid', gridTemplateColumns: '230px 170px minmax(280px, 1fr) minmax(280px, .8fr)', gap: 14, alignItems: 'start' }}>
      <aside style={{ background: C.white, border: `1px solid ${C.g200}`, borderRadius: 18, padding: 12, overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 7, overflowY: 'auto', paddingRight: 4 }}>
          {cats.map((cat) => {
            ...
            return (
              <button key={cat.id} type="button" onClick={() => onSelectCat(cat.id)} onDragOver={(event) => event.preventDefault()} onDrop={() => dropInto(selectedKind, cat.id)} style={{ width: '100%', border: `1px solid ${active ? C.light : C.g100}`, background: active ? C.bg : C.white, borderRadius: 12, padding: '10px 11px', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                  <span style={{ fontSize: 14, fontWeight: 900, color: active ? C.primary : C.g800, lineHeight: 1.35 }}>{cat.short}</span>
                  <span style={{ fontSize: 12, fontWeight: 900, color: active ? C.primary : C.g400, whiteSpace: 'nowrap' }}>{count}건</span>
                </div>
              </button>
            );
          })}
      ...
      <aside style={{ background: C.white, border: `1px solid ${C.g200}`, borderRadius: 18, padding: 12 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
          {HIERARCHY_SECTIONS.map((section) => {
            ...
            return (
              <button key={section.id} type="button" onClick={() => onSelectKind(section.id)} onDragOver={(event) => event.preventDefault()} onDrop={() => dropInto(section.id, activeCat.id)} style={{ width: '100%', border: `1px solid ${active ? C.light : C.g100}`, background: active ? C.bg : C.white, borderRadius: 13, padding: '12px 11px', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                  <span style={{ fontSize: 14, fontWeight: 900, color: active ? C.primary : C.g800 }}>{section.label}</span>
                  <span style={{ fontSize: 12, fontWeight: 900, color: active ? C.primary : C.g400 }}>{count}건</span>
                </div>
              </button>
            );
          })}
      ...
      <section style={{ background: C.white, border: `1px solid ${C.g200}`, borderRadius: 18, padding: 14, minHeight: 420 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center', marginBottom: 12 }}>
          <div>
            <div style={{ fontSize: 13, color: C.g400, fontWeight: 900, marginBottom: 3 }}>선택 파일</div>
            <div style={{ fontSize: 17, color: C.g800, fontWeight: 900 }}>{activeCat.short} / {activeSection.label}</div>
          </div>
        ...
        <div onDragOver={(event) => event.preventDefault()} onDrop={() => dropInto(activeSection.id, activeCat.id)} style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 360, overflowY: 'auto', paddingRight: 4 }}>
          ...
          {activeFiles.map((file) => (
            <div key={file.id} draggable onDragStart={() => setDragPayload({ kind: activeSection.id, catId: activeCat.id, file })} onDragEnd={() => setDragPayload(null)} onClick={() => onSelectFile(file)} style={{ display: 'grid', gridTemplateColumns: '36px minmax(0,1fr) 24px', alignItems: 'center', gap: 9, border: `1px solid ${selectedFile?.id === file.id ? C.light : C.g100}`, background: selectedFile?.id === file.id ? C.bg : '#FCFEFD', borderRadius: 12, padding: '8px 9px', cursor: 'pointer' }}>
