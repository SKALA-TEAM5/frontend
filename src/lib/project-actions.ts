...
export type ProjectActionTargetTab = 'overview' | 'upload' | 'validation' | 'actions' | 'report' | 'archive';
...
export const getAvailableProjectActions = (user: AppUser, project: ProjectSummary): ProjectAction[] => {
  ...
  if ((stageId === 'registered' || stageId === 'evidence_upload') && can(user, 'uploadEvidence')) {
    actions.push(makeAction('uploadEvidence', '증빙자료 업로드', '참여 중인 프로젝트의 정산 증빙을 제출합니다.', 'upload'));
  }

  if (stageId === 'validation' && can(user, 'runValidation')) {
  ...
  if ((stageId === 'she_review' || stageId === 'action_request') && can(user, 'requestAction')) {
    actions.push(makeAction('requestAction', '조치 요청 등록', '검증 결과에 따라 현장 보완 조치를 요청합니다.', 'actions'));
  }

  if ((stageId === 'action_request' || stageId === 'supplement_upload') && project.hasActionRequest && can(user, 'uploadEvidence')) {
    actions.push(makeAction('uploadSupplement', '보완 증빙 업로드', '요청받은 조치에 대한 보완 자료를 제출합니다.', 'upload'));
  }

  if (stageId === 'report_draft' && can(user, 'requestReport')) {
    actions.push(makeAction('requestReport', '보고서 요청', '검증 결과와 보완 현황을 바탕으로 보고서 초안을 요청합니다.', 'report'));
  }

  if ((stageId === 'report_draft' || stageId === 'report_review') && can(user, 'reviewReport')) {
    actions.push(makeAction('reviewReport', '보고서 검토', '생성된 보고서 초안을 검토하고 수정 의견을 확인합니다.', 'report'));
  }

  if (stageId === 'report_review' && can(user, 'confirmFinalReport')) {
    actions.push(makeAction('confirmFinalReport', '최종 보고서 확정', '검토가 끝난 보고서를 최종 확정합니다.', 'report'));
  }
