...
export default function ProjectStageStepper({ currentStage, compact = false }: ProjectStageStepperProps) {
    return (<div data-ui="components-common-project-stage-stepper.div-1" style={{ display: 'grid', gridTemplateColumns: `repeat(${PROJECT_STAGE_DEFINITIONS.length}, minmax(0, 1fr))`, gap: compact ? 8 : 8 }}>
      {PROJECT_STAGE_DEFINITIONS.map((stage, index) => {
            ...
            <div data-ui="components-common-project-stage-stepper.div-6" title={stage.label} style={{ fontSize: compact ? 10 : 10, fontWeight: 700, color: done || active ? C.g800 : C.g400, lineHeight: 1.2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {stage.label}
            </div>
          </div>);
        })}
